...
int isEulerian(Graph g, int numV) {
    // if a graph is eulerian iff each vertex has an even degree
    ...
    while (!allVis)
    {
        ...
        while (!isEmptyQuack(s))
        {
            ...
            if (w >= 0) {      // if true, there is an adj. vertex
                ...
                removeEdge(g, newEdge(v, w)); // remove edge to vertex
                //printf("pushed %d and remove edge %d-%d\n",w,v,w);
            }
        ...
        for (Vertex v = 0; v < numV && allVis; v++) {
            for (Vertex w = 0; w < numV && allVis; w++) {
                if (isEdge(g,newEdge(v, w))) { // any remaining edges?
